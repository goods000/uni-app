		...
		<view class="safety">
			...
			<view v-if="authInfo.authStatus == 0 || authInfo.authStatus == 3">
				...
				<!-- <view v-show="isShow"> -->
					<view class="safety-list">
						<view class="safety-list-title">真实姓名</view>
						<view class="safety-list-input">
							<input type="text" placeholder="请输入真实姓名" placeholder-style="color:#CCCCCC" v-model="username"/>
						</view>
					</view>
					<view class="safety-list">
						<view class="safety-list-title">证件号码</view>
						<view class="safety-list-input">
							<input type="text" placeholder="请输入您的证件号码" placeholder-style="color:#CCCCCC" v-model="uid"/>
						</view>
					</view>
				...
				<view>
					...
					<view class="realName">
						<view class="realName-title">请上传身份证反面照片</view>
						<view class="realName-box">
			...
			<view class="realName-end" v-if="authInfo.authStatus == 1">
				<view class="realName-status">
					<image src="../../static/exchange/icon-exchange-success.png" mode=""></image>
				</view>
			...
			<view class="realName-end" v-if="authInfo.authStatus == 2">
				...
				<view class="realName-title">通过平台审核，请放心在平台进行交易</view>
				<view class="initBtn initBtn--bottom" @click="$tools.back(1)">确认</view>
			</view>
	...
	export default {
		...
		methods: {
			getAuthInfo() {
				this.$Ajax('/front/user/authInfo', {}, res => {
					if (res.code == 0) {
						...
						if (res.obj.image1 != null && res.obj.image1 != '') {
							this.userUploadList.src1 = res.obj.image1;
						}
						if (res.obj.image2 != null && res.obj.image2 != '') {
							this.userUploadList.src2 = res.obj.image2;
						}
						if (res.obj.image3 != null && res.obj.image3 != '') {
							this.userUploadList.src3 = res.obj.image3;
						}
					}
				});
			},
			...
			unloadImg(choosePhotoPath) {
				...
				if (!/(^\d{15}$)|(^\d{17}([0-9]|X)$)/.test(this.uid)) {
					return this.$tools.toast('请输入正确的身份证号码');
				}
				if (this.uploadList[0].src == this.userUploadList[0].src) {
					return this.$tools.toast('请上传身份证正面照片');
				}
				if (this.uploadList[1].src == this.userUploadList[1].src) {
					return this.$tools.toast('请上传身份证反面照片');
				}
				uni.showLoading({
					title: '上传中',
					mask: true
				...
				console.log(files);
				uni.uploadFile({
					url: weburl.webUrl + '/front/user/approve',
					files: files,
					header: { token: token },
					formData: data,
					success: res => {
...
<style lang="scss" scoped>
	@import '/scss/safety';
	page,.pages{background:$globalColor-white;padding-bottom: 2upx;}
	// .initBtn{margin: 20% auto 5%;width: 90%;
	.initBtn{
		// margin-top: 40%;position: absolute;bottom: 46upx;left: 0;right: 0;margin: auto;
		&_upload{
			// position: absolute;
			// bottom: 4%;
			// left: 0;right: 0;
