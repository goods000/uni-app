	...
	<view class="pages">
		<view class="initHeader initHeader--line initHeader--black">
			<view class="initHeader-wrapper">
				...
				<view class="initHeader-title">支付宝收款账号</view>
			</view>
		</view>
				...
				<view class="uploadBox-wrapper">
					...
					<view class="uploadBox-form">
						<view class="uploadBox-list">
							<view class="uploadBox-list__label">支付宝账号</view>
							<view class="uploadBox-list__data">
								<input placeholder-class="uploadBox-placeholder" placeholder="请输入需要绑定的支付宝收款账号" v-model="cardId" />
							</view>
						</view>
						<view class="uploadBox-list">
							<view class="uploadBox-list__label">真实姓名</view>
							<view class="uploadBox-list__data">
								<input placeholder-class="uploadBox-placeholder" placeholder="请输入支付宝账号实名认证的姓名" v-model="name" />
							</view>
						</view>
					</view>
				</view>
...
export default {
	data() {
		return {
			...
			name: '',
			cardId: '',
			src1: '../../static/payment/img-upload-alipay.png',
			demoSrc: '../../static/payment/img-upload-alipay.png',
			updata: 1
		};
	},
	onShow() {
	...
	methods: {
		...
		uploadImg(src) {
			this.updata = 0; //判断是否更改图片

			uni.setStorageSync('idCardName_alipay', this.name);
			uni.setStorageSync('idCardId_alipay', this.cardId);
			...
			uni.chooseImage({
				...
				complete: function() {
					if (uni.getStorageSync('idCardName_alipay')) {
						this.name = uni.getStorageSync('idCardName_alipay');
					}
					
					if (uni.getStorageSync('idCardId_alipay')) {
						this.cardId = uni.getStorageSync('idCardId_alipay');
					}
				}
			});
		},
		...
		bindFunc() {
			...
			if (this.cardId == '') {
				return this.$tools.toast('请输入支付宝账号');
			}
			if (this.name == '') {
				return this.$tools.toast('请输入支付宝账号实名认证的姓名');
			}
