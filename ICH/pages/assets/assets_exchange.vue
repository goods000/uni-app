		...
		<view class="main">
			...
			<view class="inputBox">
				...
				<view class="inputBox-list">
					<view class="inputBox-list__input"><input type="text" placeholder-class="inputBox-list__placeholder" placeholder="请输入兑换的数量" v-model="quantity" /></view>
			...
			<view class="initButton"><view class="initButton-btn initButton-btn--blue" @click="transter()">确定</view></view>
		</view>
				...
				res => {
					if (res.code == 0) {
						...
						if (this.conversionType == 0) {
							...
							if (assetType == 3) {
								this.using = this.walletList.otcUsing;
							} else if (assetType == 4) {
								this.using = this.walletList.bondUsing;
							} else if (assetType == 2) {
								this.using = this.walletList.using;
							}
						}
...
.conversionBox {
	...
	&-button {
		flex-basis: 120upx;
		background: url('../../static/assets/icon-conversion.png') no-repeat;
