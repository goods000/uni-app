	...
	<view class="pages">
		...
		<view class="listBox">
			<view class="listBox-wrapper">
				...
				<view class="listBox-list" v-for="(item, index) in recordList" :key="index" @click="$tools.jump('../assets/assets_transfer_detail', item.id)" v-else>
					...
					<view class="listBox-list__body">
						<view class="listBox-list__item">
							<view class="listBox-list__item-label">块高({{ item.type == 1 ? '转出' : '转入' }})</view>
					...
					<view class="listBox-list__footer">
						<view class="listBox-list__date">
							<view class="listBox-list__date-data">{{ item.type == 1 ? '转出地址：' : '转入地址：' }}{{ item.type == 1 ? item.fromAddress : item.toAddress }}</view>
						</view>
					</view>
				</view>
			</view>
