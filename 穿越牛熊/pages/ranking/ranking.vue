<template>
	<view class="pages" :data-theme="pageTheme">
		...
		<!-- 背景 -->
		<view class="initBg"><image :src="'../../static/ranking/img-bg-' + pageTheme + '.png'" mode="widthFix"></image></view>
			...
			<view class="rankingListBox">
				<view class="rankingListBox-wrapper">
					<view class="rankingListBox-header">
						<view class="rankingListBox-data" v-if="tabsActive != 2">排名</view>
						<view class="rankingListBox-data" v-if="tabsActive == 2">日期</view>
					...
					<view class="rankingListBox-body">
						...
						<view class="rankingListBox-list" v-for="(item, index) in rankingList" :key="index">
							<view class="rankingListBox-data rankingNumBox" v-if="tabsActive != 2">
							...
							<view class="rankingListBox-data" v-if="tabsActive == 2">
								<view class="rankingNumBox-data">{{ item.month }}/{{ item.day }}</view>
							</view>
							...
							<view class="rankingListBox-data rankingListBox-data--flex4 userInfoBox">
								<view class="userInfoBox-wrapper">
									...
									<view class="userInfoBox-name">{{ item.name | conceal(3) }}</view>
								</view>
							</view>
				...
				res => {
					...
					if (res.code == 0) {
						if(res.obj.list != null){
							...
							this.rankingList = this.rankingList.concat(res.obj.list);
