	...
	<view class="pages">
		...
		<view class="typeBox">
			<view class="typeBox-wrapper">
				<view class="typeBox-title"><view class="typeBox-title__label">类型</view></view>

				<radio-group @change="radioChange" class="typeBox-body">
					<view class="initNoData" v-if="typeList.length == 0">暂无数据</view>
					<label class="typeBox-list" v-for="(item, index) in typeList" :key="index">
						<view class="typeBox-list__lable">{{ item.name }}</view>
						<view class="typeBox-list__radio"><radio :value="String(item.id)" :checked="item.id === current" color="#2537d8" style="transform:scale(0.7)" /></view>
					</label>
				</radio-group>
			</view>
		</view>
		...
		<view class="initButton initButton--absolute">
			<view class="initButton-btn initButton-btn--style" @click="$tools.jump('../workOrder/workOrder_apply', current)">下一步</view>
		</view>
	</view>
</template>
