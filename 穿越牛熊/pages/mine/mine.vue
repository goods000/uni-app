					...
					<view class="assetsBox">
						<view class="assetsBox-wrapper">
							<view class="assetsBox-content">
								<view class="assetsBox-label">
									总收益(USDT)
									<image :src="eyeType ? '../../static/mine/icon-user-see.png' : '../../static/mine/icon-user-nosee.png'" @click="toggleAssetsInfo()" mode="widthFix"></image>
								</view>
								<view class="assetsBox-price" v-if="isLogin">{{ eyeType ? userInfo.totalProfit + userInfo.totalReward : '---' | number(4) }}</view>
								<view class="assetsBox-price" v-else>---</view>
							</view>
							<view class="assetsBox-itemBox">
								<view class="assetsBox-itemBox__item">
									<view class="assetsBox-itemBox__item-label">量化收益</view>
									<view class="assetsBox-itemBox__item-data" v-if="isLogin">{{ eyeType ? userInfo.totalProfit : '---' | number(4) }}</view>
									<view class="assetsBox-itemBox__item-data" v-else>---</view>
								</view>
								<view class="assetsBox-itemBox__item">
									<view class="assetsBox-itemBox__item-label">返佣收益</view>
									<view class="assetsBox-itemBox__item-data" v-if="isLogin">{{ eyeType ? userInfo.totalReward : '---' | number(4) }}</view>
									<view class="assetsBox-itemBox__item-data" v-else>---</view>
								</view>
							</view>
						</view>
					...
					<view class="fancyBox">
						<view class="fancyBox-wrapper">
							<view class="fancyBox-info">
								...
								<view class="fancyBox-data">{{ isLogin ? userInfo.service : '---' | number(4) }} {{ $store.state.projectCurrencyName }}</view>
							</view>
							<view class="fancyBox-body">
								...
								<view class="fancyBox-item" @click="$tools.jump('../assets/assets_withdraw')"><view class="fancyBox-item__label">提币</view></view>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="listBox">
				<view class="listBox-wrapper">
					<view class="listBox-plane">
						...
						<view class="listBox-list" @click="$tools.jump('../activation/activation')">
							<view class="listBox-list__icon"><image :src="'../../static/mine/icon-item-00-' + pageTheme + '.png'" mode="widthFix"></image></view>
							<view class="listBox-list__label">激活账户</view>
						</view>
...
export default {
	...
	onShow() {
		...
		if(!this.isLogin){
			this.eyeType = false
			uni.setStorageSync('eyeType', this.eyeType);
		}else{
