<template>
	<view class="pages" :data-theme="pageTheme">
					...
					<view class="headerInfoBox">
						<view class="headerInfoBox-wrapper">
							...
							<view class="headerInfoBox-add">
								<view class="headerInfoBox-add__icon" v-if="userActive == 0" @click="$tools.toast('请先激活')"></view>
								<view class="headerInfoBox-add__icon" v-else @click="toolsPopupVisible = !toolsPopupVisible"></view>
			...
			<view class="listBox">
				<view class="listBox-wrapper">
					<view class="listBox-body">
						<view class="initNoData" v-if="myRobotList.length == 0">暂无绑定交易对</view>
						<view class="listBox-list" v-for="(item, index) in myRobotList" :key="index">
							<view class="listBox-list__plane">
								<view class="listBox-list__header">
									...
									<view class="listBox-list__more" v-if="userActive == 0" @click="$tools.toast('请先激活')"></view>
									<view class="listBox-list__more" v-else @click="openTools2Popup('toolsPopup', item)"></view>
								</view>
									...
									<view class="itemBox-wrapper">
										...
										<view class="itemBox-item">
											<view
												...
												:class="[
													{ 'itemBox-item__data--green': item.rise > 0 && item.orderTime != 0 },
													{ 'itemBox-item__data--red': item.rise < 0 && item.orderTime != 0 }
												]"
											>
												{{ item.orderTime == 0 ? 0 : item.rise | number(4) }}%
											</view>
							...
							<view class="listBox-list__dropDownBox dropDownBox" v-if="dropdownType == index">
								<view class="dropDownBox-wrapper">
									...
									<view class="dropDownBox-recordBox" v-if="item.orderList.length != 0">
										<view class="dropDownBox-recordBox__item" v-for="(subItem, subIndex) in item.orderList" :key="subIndex">
											<view class="dropDownBox-recordBox__left">
												<view class="dropDownBox-recordBox__column">
													<view class="dropDownBox-recordBox__label">时间</view>
													<view class="dropDownBox-recordBox__data">{{ subItem.createTime }}</view>
												</view>
											</view>
											<view class="dropDownBox-recordBox__center">
												<view class="dropDownBox-recordBox__row">
													<view class="dropDownBox-recordBox__label">数量：</view>
													<view class="dropDownBox-recordBox__data">{{ subItem.quantity | number(6) }}</view>
												</view>
												<view class="dropDownBox-recordBox__row">
													<view class="dropDownBox-recordBox__label">单价：</view>
													<view class="dropDownBox-recordBox__data">{{ subItem.price | number(6) }}</view>
												</view>
											</view>
											<view class="dropDownBox-recordBox__right">
												<view class="dropDownBox-recordBox__btn" @click="openPopup('manualPopup', item, subItem)">
													<view class="dropDownBox-recordBox__btn-icon">
														<image :src="'../../static/quantification/icon-manual-' + pageTheme + '.png'" mode="widthFix"></image>
													</view>
													<view class="dropDownBox-recordBox__btn-label">手动补仓</view>
												</view>
											</view>
										</view>
									</view>
				...
				<view class="toolsBox-wrapper">
					...
					<view class="toolsBox-itemBox">
						...
						<view class="toolsBox-itemBox__item" @click="hideTools2Popup('toggleStatusPopup')">
							<view class="toolsBox-itemBox__item-icon">
								<image
									:src="'../../static/quantification/icon-tools-02-' + (popupInfo.status == 1 ? 'pause' : 'start') + '-' + pageTheme + '.png'"
									mode="widthFix"
								></image>
							</view>
		...
		<!-- 手动补仓 -->
		<uni-popup ref="manualPopup" type="center" :animation="true" @change="changePopup">
			<view class="initPopup">
				<view class="initPopup-wrapper">
					<view class="initPopup-title">手动补仓</view>
					<view class="initPopup-listBox">
						<view class="initPopup-listBox__list">
							<view class="initPopup-listBox__list-input">
								<input type="text" placeholder="请输入加仓数量" placeholder-class="initPopup-listBox__list-placeholder" v-model="manualQuantity" />
							</view>
						</view>
						<view class="initPopup-listBox__note">当前火币可用：{{ assetInfo.bourseAsset | number(6) }} USDT</view>
					</view>
					<view class="initPopup-buttonBox">
						<view class="initPopup-buttonBox__btn initPopup-buttonBox__btn--cancel" @click="hidePopup('manualPopup')">取消</view>
						<view class="initPopup-buttonBox__btn initPopup-buttonBox__btn--confirm" @click="manualOrder()">确认</view>
					</view>
				</view>
			</view>
		</uni-popup>
...
export default {
	...
	methods: {
		...
		// 用户激活状态
		getUserActive() {
			this.$Ajax('/front/user/userActive', {}, res => {
				if(res.code == 0){
					this.userActive = res.obj;
				}
			})
		},
		getApiAssets() {
		...
		getMyRobotList() {
			...
			this.$Ajax('/front/utrade/user/list', {}, res => {
				...
				if (res.code == 0) {
					this.tempMyRobotList = res.obj;
					this.myRobotList = JSON.parse(JSON.stringify(this.tempMyRobotList));
				}
			});
		},
		getAccountList() {
		...
		manualOrder(){
			console.log(this.popupInfo.id,this.popupSubInfo.id,this.manualQuantity)
			if(this.manualQuantity == ''){
				this.$tools.toast('请输入补仓金额');
			}
			
			/* 交易对手动补仓 */
			this.$Ajax(
				'/front/utrade/isolate/order',
				{
					id: this.popupInfo.id,
					orderId: this.popupSubInfo.id,
					money: this.manualQuantity
				},
				res => {
					console.log(JSON.stringify(res));
					if (res.code == 0) {
						this.$tools.toast('补仓成功');
						this.hidePopup('manualPopup');
						this.manualQuantity = '';
						setTimeout(() => {
							this.getMyRobotList();
							this.getApiAssets();
		...
		setTradeData(data) {
			const filterData = (item) => item.id == data.obj.id;
			let myRobotDataIndex = this.tempMyRobotList.findIndex(filterData);
			
			if (myRobotDataIndex != -1) {
				this.tempMyRobotList[myRobotDataIndex].currentPrice = data.obj.price;
				this.tempMyRobotList[myRobotDataIndex].amount = data.obj.amount;
				this.tempMyRobotList[myRobotDataIndex].rise = data.obj.rise;
			}
		},
		subTradeMessage() {
			var sendInfo = {};
		...
		/* 弹出框公用 */
		openPopup(e, item, subItem) {
			if (item) {
				this.popupInfo = item;
			}
			if(subItem){
				this.popupSubInfo = subItem;
			}
			uni.hideTabBar();
			this.$refs[e].open();
		},
