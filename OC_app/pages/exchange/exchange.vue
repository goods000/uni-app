							...
							<view class="platformBox-sheetContent">
								<view class="platformBox-resourceBox platformBox-resourceBox--sell" v-show="planeVal == 0 || planeVal == 2">
									<view class="platformBox-resourceBox__wrapper">
										<view
											...
											v-for="(item, sellIndex) in parseInt((planeVal == 0 ? 5 : 10) - sellList.slice(planeVal == 0 ? -5 : -10).length)"
											:key="'sell-' + sellIndex"
										>
								...
								<view class="platformBox-resourceBox  platformBox-resourceBox--buy" v-show="planeVal == 0 || planeVal == 1">
									<view class="platformBox-resourceBox__wrapper">
										...
										<view
											class="platformBox-resourceBox__list platformBox-resourceBox__list--green"
											v-for="(item, buyIndex) in parseInt((planeVal == 0 ? 5 : 10) - buyList.slice(0, planeVal == 0 ? 5 : 10).length)"
											:key="'buy-' + buyIndex"
										>
											<view class="platformBox-resourceBox__content">
												<view class="platformBox-resourceBox__price">--</view>
...
import URLSET from '@/common/js/url.js';
...
export default {
	...
	methods: {
		...
		sliderChange(e) {
			...
			if (this.buttonActive == 0) {
				if (this.trustVal == 2) {
						num = (this.walletList.using * this.sliderVal) / this.price / 100;
				} else {
						num = (this.walletList.using * this.sliderVal) / this.tradeInfo.currentPrice / 100;
				}
			} else {
				num = (this.tradeWalletList.using * this.sliderVal) / 100;
			}
			let numStr = num.toString();
			let index = numStr.indexOf('.');
			let result = numStr.slice(0, index + 5); //控制小数位的浮点问题
			this.quantity = Number(result);
		},
...
@import '@/common/scss/pages/exchange/exchange';
