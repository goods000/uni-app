<template>
	<view class="page">
		...
		<view class="page-title-blue">{{ i18n.login.bindPhone }}:{{phone}}</view>
		<view class="fill-plate">
			<view class="input-plate">
				<view class="input-title">{{ i18n.login.newPwd }}</view>
				<view class="input-flex">
					<input :password="!eyeShowHideArray[0].show" v-model="tradePwd" value="" :placeholder="i18n.login.placeholder_02" />
			...
			<view class="input-plate">
				...
				<view class="input-flex">
					<input :password="!eyeShowHideArray[1].show" v-model="againTradePwd" value="" :placeholder="i18n.login.placeholder_03" />
			...
			<view class="input-plate">
				<view class="input-title">{{ i18n.global.code }}</view>
				<view class="input-flex">
					<input type="text" v-model="code" value="" :placeholder="i18n.global.placeholder_code" />
					<view class="flex-right" @click="sendCode()">{{remain}}</view>
				</view>
			</view>
		</view>
	</view>
...
<script>
	export default {
		...
		methods:{
			...
			next(){
				if(this.tradePwd==''){
					this.$tools.toast(this.i18n.login.placeholder_02);
					return
				}
				if(this.againTradePwd==''){
					this.$tools.toast(this.i18n.login.placeholder_03);
					return
				}
				if(this.code==''){
					this.$tools.toast(this.i18n.global.placeholder_code);
					return
				}
				if(this.tradePwd!=this.againTradePwd){
					this.$tools.toast(this.i18n.login.placeholder_04);
					return
				}
				this.$Ajax('/front/user/modify/tradePwd',{
					...
					console.log(res);
					if(res.code==0){
						this.$tools.toast(this.i18n.global.modifySuccess);
						this.$tools.back(1);
					}
				})
			},
