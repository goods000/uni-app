<template>
	<view class="gesture-lock" :class="{error:error}" :style="{width: containerWidth +'rpx', height:containerWidth +'rpx'}"
	 @touchstart.stop="onTouchStart" @touchmove.stop="onTouchMove" @touchend.stop="onTouchEnd" @touchstart.stop.prevent="moveHandle">
		...
		<!-- 9 个圆 -->
		<view>
			<view v-for="(item,i) in circleArray" :key="i" class="cycle" :class="{check:item.check,error: error}" :style="{left:item.style.left,top:item.style.top,width:item.style.width,height:item.style.width}">
			</view>
		</view>
		<view>
			<!-- 已激活锁之间的线段 -->
...
<style lang="scss" scoped>
	...
	.gesture-lock .cycle {
		box-sizing: border-box;
		position: absolute;
		border: 2px solid #fad668;
		border-radius: 50%;
		opacity: 0.5;
	}
	
	// .gesture-lock .cycle.error:after{
