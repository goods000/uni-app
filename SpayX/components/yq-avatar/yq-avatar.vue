			...
			<view class="oper">
				<view class="btn-wrapper" v-if="showOper">
					<view class="back" @tap="back">
						<!-- <image src="../../static/backs.png" mode=""></image> -->
						
					</view>
					<view class="submit" @click="fUpload"><text>确定</text></view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.qlty = parseInt(this.quality) || 0.5;
		...
		methods: {
			...
			fWindowResize() {
				...
				this.cvsStyleHeight = this.windowHeight + 'px';
			...
			fUpload() {
				...
				return this.$tools.toast('图像修改成功')
			},
			fDrawImage() {
				let tm_now = Date.now();
			...
			fDrawInit(ini=false) {
				...
				if( ini ) {
					...
					// #ifdef H5
					this.styleTop = this.drawTop + 50 + 'px';
					// #endif
					// #ifndef H5
					this.styleTop = '0';
					// #endif
