	...
	<view class="pages contentBg">
		<view class="loginBox">
			...
			<view class="loginBox-warpper">
				<view class="loginBox-warpper-title">重设密码</view>
		...
		<view class="content">
			<view class="content-warpper">
				<view class="content-warpper-title content-warpper-title-mt">新密码</view>
				<view class="content-warpper-input">
					<input :password="!eyeShowHideArray[0].show" placeholder="重新设置您的密码" placeholder-class="inputColor" v-model="newPassword"
		...
		<view class="operateBtn">
			<view class="operateBtn-btn" @click="config()">确认</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			config() {
				...
				this.$Ajax('/front/forgetPassword',{
					...
					res => {
						...
						if (res.code == 0) {
							this.$tools.toastJump('重设成功', './login');
						}
					}
				);
			}
