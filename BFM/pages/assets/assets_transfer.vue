			...
			<view class="formBox">
				<view class="formBox-wrapper">
					<view class="formBox-list">
						<view class="formBox-list__label">接收账号</view>
						<view class="formBox-list__input">
							<input type="text" placeholder-class="formBox-placeholder" placeholder="输入对方的接收账号" v-model="address" />
	...
	methods: {
		...
		getCurrencyInfo() {
			this.$Ajax(
				...
				res => {
					console.log('钱包信息：' + JSON.stringify(res));
					if (res.code == 0) {
		...
		getFeeData() {
			this.$Ajax(
				...
				res => {
					console.log(JSON.stringify(res.obj));
					if (res.code == 0) {
						this.feeRate = res.obj.rate;
						this.percentage = res.obj.percentage;
					}
				}
			);
		},
