			...
			<view class="formBox">
				<view class="formBox-wrapper">
					...
					<view class="formBox-list">
						...
						<view class="formBox-list__input">
							<input type="number" placeholder-class="formBox-placeholder" :placeholder="'最低提币' + currencyInfoList.min + '个'" v-model="quantity" />
					...
					<view class="formBox-list">
						...
						<view class="formBox-list__note">向{{ phoneStr | conceal(3) }}发送验证码</view>
						<view class="formBox-list__input">
							<input type="number" maxlength="6" placeholder-class="formBox-placeholder" placeholder="请输入验证码" v-model="code" />
					...
					<view class="formBox-list">
						...
						<view class="formBox-list__input"><input type="password" placeholder-class="formBox-placeholder" placeholder="请输入交易密码" v-model="tradePsw" /></view>
					</view>

					<view class="formBox-list">
						<view class="formBox-list__tips">温馨提示</view>
						<view class="formBox-list__note">最小提币数量为： {{ currencyInfoList.min }} {{ currencyLabel }}。（备注：为了确保链上数据正常，提币以10的倍数整数为提)</view>
						<view class="formBox-list__note">为了保证资金安全，当您的账户安全策略变更，密码修改，我们会对人工审核，请耐心等待。</view>
						<view class="formBox-list__note">请务必确认电脑及浏览器安全，防止信息被篡改或泄露。</view>
					</view>

					<view class="initButton"><view class="initButton-btn initButton-btn--style" @click="apply()">提币</view></view>
				</view>
			</view>
		</view>
...
export default {
	...
	data() {
		return {
			...
			using: '', //可提数量
			percentage: '', //提币规则是否百分比(百分比:提币数量*rate,不为百分比:提币数量-rate)
			rate: '', //提币扣除手续费
			min: 0,
			max: 0,
			temporary: '', //临时
		...
		quantity() {
			if (this.quantity == '') {
				...
				if (this.currencyInfoList.percentage) {
					this.charge = this.quantity * (this.currencyInfoList.rate / 100);
				} else {
	...
	methods: {
			...
			this.$Ajax(
				...
				res => {
					...
					if (res.code == 0) {
						...
						if (this.pageType == 1) {
							this.currencyArray = data.filter(item => {
								return item.name == 'FIL';
							});
						} else {
		...
		apply() {
			...
			if (this.quantity % 10 != 0) {
				return this.$tools.toast('请输入10的倍数的数量');
			}
