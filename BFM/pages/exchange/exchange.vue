<template>
	...
	<view class="pages">
			...
			<view class="initHeader-wrapper">
				<!-- <view class="initHeader-tabs">
					<view class="initHeader-tabs__wrapper">
						<view class="initHeader-tabs__item initHeader-tabs__item--active">币币交易</view>
						<view class="initHeader-tabs__item" @click="$tools.noOpen()">法币交易</view>
					</view>
				</view> -->
				<view class="initHeader-title">交&nbsp;易</view>
			</view>
		</view>
		...
		<view class="initHeader initHeader--style">
			<view class="initHeader-wrapper">
			...
			<view class="platformBox-wrapper">
				<view class="platformBox-content">
					...
					<view class="platformBox-sheetBox rightBigBox">
						<view class="platformBox-sheetMain">
							...
							<view class="platformBox-sheetContent">
								<view class="platformBox-resourceBox platformBox-resourceBox--sell" v-show="planeVal == 0 || planeVal == 2">
									<view class="platformBox-resourceBox__wrapper">
										<view class="platformBox-resourceBox__list platformBox-resourceBox__list--red" v-for="(item, sellIndex) in parseInt((planeVal == 0 ? 5 : 10) - sellList.slice(planeVal == 0 ? -5 : -10).length)" :key="'sell-' + sellIndex">
											<view class="platformBox-resourceBox__content">
												<view class="platformBox-resourceBox__price">--</view>
												<view class="platformBox-resourceBox__quantity">--</view>
											</view>
										</view>
										<view
								...
								<view class="platformBox-resourceBox  platformBox-resourceBox--buy" v-show="planeVal == 0 || planeVal == 1">
									<view class="platformBox-resourceBox__wrapper">
										...
										<view class="platformBox-resourceBox__list platformBox-resourceBox__list--green" v-for="(item, buyIndex) in parseInt((planeVal == 0 ? 5 : 10) - buyList.slice(0, planeVal == 0 ? 5 : 10).length)" :key="'buy-' + buyIndex">
											<view class="platformBox-resourceBox__content">
												<view class="platformBox-resourceBox__price">--</view>
												<view class="platformBox-resourceBox__quantity">--</view>
											</view>
		...
		<view class="recordBox">
			<view class="recordBox-nav">
				...
				<view class="recordBox-nav__record" @click="$tools.jump('../exchange/exchange_record', currencyTradeName, tradeId)">更多&gt;&gt;</view>
			</view>
			<view class="recordBox-wrapper">
				<view class="recordBox-content">
					<view class="ticketBox">
						<view class="ticketBox-wrapper">
							<view class="initNoData" v-if="nowEntrustList.length == 0">暂无记录</view>
							<view class="ticketBox-list" v-for="(item, index) in nowEntrustList" :key="index" v-else>
								...
								<view class="ticketBox-itemBox">
									<view class="ticketBox-itemBox__wrapper">
										...
										<view class="ticketBox-itemBox__item">
											<view class="ticketBox-itemBox__item-label">数量({{ item.tradeCurrencyName }})</view>
											<view class="ticketBox-itemBox__item-data">{{ item.quantity }}</view>
										</view>
										<view class="ticketBox-itemBox__item">
											<view class="ticketBox-itemBox__item-label">实际成交({{ item.currencyName }})</view>
											<view class="ticketBox-itemBox__item-data">{{ (item.price * item.quantity) | number(6) }}</view>
										</view>
									</view>
								</view>
							</view>
...
export default {
	...
	methods: {
		...
		init() {
			...
			let tradeId = uni.getStorageSync('tradeId');
		...
		getAreaList() {
			this.$Ajax('/search/indexArea', {}, res => {
				console.log('区域信息' + JSON.stringify(res));
				if (res.code == 0) {
					this.curAreaId = res.obj[0].id;
				}
			});
		},
		getTradeId(){
			this.$Ajax('/search/indexTrade', {
				areaId: this.curAreaId
			}, res => {
				console.log('币种信息' + JSON.stringify(res));
				if (res.code == 0) {
					this.tradeId = res.obj[0].tradeId;
					uni.setStorageSync('tradeId', res.obj[0].tradeId);
					// this.curAreaId = res.obj[0].id;
				}
			});
		},
		...
		clearTime() {
			...
			if (this.trade_timer != null) {
				clearInterval(this.trade_timer);
				this.trade_timer = null;
			}
			if (this.record_timer != null) {
				clearInterval(this.record_timer);
