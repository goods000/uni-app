import messagesCn from './language/cn.js';
import messagesEn from './language/en.js';
...
tools.copy = (val) => {
	uni.setClipboardData({
		...
		success() {
			uni.showToast({
				title: uni.getStorageSync('initlanguage') == 'cn' ? messagesCn.global.copySuccess : messagesEn.global.copySuccess,
				duration: 2000,
				icon: "none"
			})
		}
...
tools.jump = (path, value1, value2, value3, value4, value5, value6, value7) => {
	var string = '';
...
tools.noOpen = () => {
	uni.showToast({
		title: uni.getStorageSync('initlanguage') == 'cn' ? messagesCn.global.comingSoon : messagesEn.global.comingSoon,
		icon: "none",
		mask: true,
		duration: 1000,
		position: 'bottom'
