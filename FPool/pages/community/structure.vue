	...
	<view class="contentBg">
		<view class="header_bar">
			<view class="header">
				...
				<view class="header-title">{{ i18n.user.list_01 }}</view>
			</view> 
		</view> 
		...
		<view class="community">
			<view class="communityBox">
				<view class="communityBox-list">
					<view class="communityBox-list-title">{{ i18n.team.team_total }}</view>
					<view class="communityBox-list-price">{{ myTeam.teamPeopleCount }} {{ i18n.user.people }}</view>
				</view>
			</view>
			...
			<view class="communityBox-box">
					...
					<view class="tipsBox">
						<view class="tipsBox-wrapper" @click="triggerCollapse(0)">
							<view class="tipsBox-title">
								<view class="tipsBox-title__label">{{ i18n.team.direct_push }}</view>
								<view class="tipsBox-right">
									<view class="tipsBox-right-title">{{ myTeam.vipCount_1 }} {{ i18n.user.people }}</view>
								</view>
							</view>
						</view>
					</view>
					<view class="communityBox-box-list__item" v-if="navigateFlag[0].show">
						<view class="initNoData" v-if="myTeamList_1 == null">{{ i18n.global.noData }}</view>
						<view class="communityBox-box-list__item-type" v-for="(item,index) in myTeamList_1" :key="index" v-else>
							...
							<view class="communityBox-box-list__item-title">{{item.account.substring(0,3) + "****" + item.account.substring(7)}}</view>
				...
				<!-- <view class="communityBox-box-list">
					...
					<view class="communityBox-box-list__item" v-if="navigateFlag[1].show">
						<view class="initNoData" v-if="myTeamList_2 == null">{{ i18n.global.noData }}</view>
				...
				<view class="communityBox-box-list">
					...
					<view class="communityBox-box-list__item" v-if="navigateFlag[2].show">
						<view class="initNoData" v-if="myTeamList_3 == null">{{ i18n.global.noData }}</view>
