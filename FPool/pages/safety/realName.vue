		...
		<view class="safety">
			<view class="header_bar">
				<view class="header">
					...
					<view class="header-title">{{ i18n.safety.list_01 }}</view>
				</view> 
			</view> 
			...
			<view v-if="authInfo.authStatus == 0 || authInfo.authStatus == 3">
				<view v-show="isShow">
					<view class="safety-list">
						<view class="safety-list-title">{{ i18n.realName.name }}</view>
						<view class="safety-list-input">
							<input type="text" :placeholder="i18n.realName.placeholder_01" placeholder-style="color:#999999" v-model="username"/>
						</view>
					</view>
					<view class="safety-list">
						<view class="safety-list-title">{{ i18n.realName.idCard }}</view>
						<view class="safety-list-input">
							<input type="text" :placeholder="i18n.realName.placeholder_02" placeholder-style="color:#999999" v-model="uid"/>
						</view>
					</view>
					<view class="initBtn" @click="next()">{{ i18n.global.next }}</view>
				</view>
				
				<view v-show="!isShow">
					<view class="realName">
						<view class="realName-title">{{ i18n.realName.tips_01 }}</view>
					...
					<view class="realName">
						<view class="realName-title">{{ i18n.realName.tips_02 }}</view>
						<view class="realName-box">
					...
					<view class="initBtn initBtn_upload" @click="unloadImg()">{{ i18n.global.sumbit }}</view>
				</view>
			</view>
			...
			<view class="realName-end" v-if="authInfo.authStatus == 1">
				...
				<view class="realName-result">{{ i18n.realName.status_01 }}</view>
				<view class="realName-title">{{ i18n.realName.status_02 }}</view>
				<view class="realName-title">{{ i18n.realName.status_03 }}</view>
			</view>
			...
			<view class="realName-end" v-if="authInfo.authStatus == 2">
				...
				<view class="realName-result">{{ i18n.realName.status_04 }}</view>
				<view class="realName-title">{{ i18n.realName.status_05 }}</view>
				<view class="realName-title">{{ i18n.realName.status_06 }}</view>
				<view class="initBtn" @click="$tools.back(1)">{{ i18n.global.confirm }}</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			next(){
				if (this.username == '') {
					return this.$tools.toast(this.i18n.realName.toast_01);
				}
				if (this.uid == '') {
					return this.$tools.toast(this.i18n.realName.toast_02);
				}
				this.$Ajax(
			...
			unloadImg(choosePhotoPath) {
				...
				if (this.uploadList[0].src == this.userUploadList[0].src) {
					return this.$tools.toast(this.i18n.realName.tips_01);
				}
				if (this.uploadList[1].src == this.userUploadList[1].src) {
					return this.$tools.toast(this.i18n.realName.tips_02);
				}
				uni.showLoading({
					title: this.i18n.realName.toast_04,
					mask: true
				});
				...
				uni.uploadFile({
					...
					success: res => {
						...
						console.log(data);
						if (data.code == 0) {
							that.$tools.toastBack(this.i18n.realName.toast_03, 1);
						} else {
