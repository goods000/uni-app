		...
		<view class="form">
			...
			<view class="form-box mt-mb">
				...
				<view class="form-group">
					...
					<input type="text" placeholder="" :value="send_slc_fee_scale + 'USDT'" disabled />
				</view>
				<view class="form-group">
			...
			<view class="btn-active-login" @click="submit()" v-if="send_slc_switch == 0 && send_mx_switch == 0 && send_usdt_switch ==0">确定</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			getCash() {
				this.$Ajax('/deal/index', {}, res => {
						if (res.code == 0) {
							this.send_slc_fee_scale = res.send_slc_fee_scale
							this.send_slc_switch = res.send_slc_switch
							this.send_mx_switch = res.send_mx_switch
							this.send_usdt_switch = res.send_usdt_switch
						}
					}
				);
			},
		...
		computed:{
			actual_num() {
				var a_num = this.num - this.send_slc_fee_scale;
				if(a_num <= 0){
					return 0;
				}else{
					return a_num;
