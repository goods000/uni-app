	...
	<view class="pages">
		<view class="header_bar header_border">
			<view class="header">
				...
				<view class="header-title">{{ type == '' ? '绑定' : '修改' }}地址</view>
			</view> 
		</view> 
		...
		<view class="content">
			<view class="content-warpper">
				<view class="content-warpper-input">
					<input type="text" placeholder="请输入您的地址" placeholder-class="inputColor" v-model="addType"
...
<script>
	export default {
		...
		methods: {
			...
			config() {
				if (this.addType == '') {
					return this.$tools.toast('请输入您的地址');
				}
				this.$Ajax('/front/user/updateUser',{
						address: this.addType,
					},
					res => {
