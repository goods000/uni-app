		...
		<view class="main">
			...
			<view class="echartBox">
				<view class="echartBox-wrapper">
					<view @click="echarts.onClick" :prop="option" :change:prop="echarts.updateEcharts" id="networkHoldingEcharts" class="echartBox-plane"></view>
				</view>
			</view>
		</view>
	</view>
				...
				res => {
					...
					if (res.code == 0) {
						...
						res.obj.forEach((item, index) => {
							...
							seriesHBData.push(item.huobiAmount);
							seriesOKData.push(item.okexAmount);
							seriesBITData.push(item.bitmexAmount);
						});

						this.option.xAxis.data = xAxisData;
						this.option.series[0].data = seriesHBData;
						this.option.series[1].data = seriesOKData;
						this.option.series[2].data = seriesBITData;
					}
				}
			);
		},
...
export default {
	...
	methods: {
		initEcharts() {
			myChart = echarts.init(document.getElementById('networkHoldingEcharts'))
